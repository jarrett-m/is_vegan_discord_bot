...
# Bind commands.DefaultHelpCommand to help_command.
help_command = commands.DefaultHelpCommand(
    no_category = 'Commands'
)

# Binds activity to activity to show help command.
...
# Page animation class for v?find.
class MySource(menus.ListPageSource):
    def __init__(self, data):
        '''Default listing per page = 8.'''
        super().__init__(data, per_page=8)

    async def format_page(self, menu, entries):
        '''Format each page and sends items found page by page.'''
...
def combine_words(arg):
    '''
       ...
       Typically combine user input to search.
    '''
    if len(arg) > 1:
...
async def vegan(ctx, *ingredient):
    ...
    '''
    # from *is_vegan import* and *can_be_vegan*. 
    # Bot returns true false or maybe.
...
@bot.command()
async def ingredents(ctx, p_id):
    ingredients = get_ingredents(int(p_id))
    string = ''
